...
local function get_diags(buflist, hl_base)
	...
	for _, ty in ipairs({ "Error", "Warn", "Info", "Hint" }) do
		...
		if n > 0 then
			diags[#diags + 1] = ("%%#Diagnostic%s%s#%s%s"):format(
				ty,
				hl_base,
				icons[ty],
				n
...
M.tabline = function()
	...
	for i = 1, fn.tabpagenr("$") do
		...
		--- @type integer
		local width =
			api.nvim_eval_statusline(part, { use_tabline = true }).width
...
vim.opt.tabline = "%!v:lua.require'plugins.tab'.tabline()"
