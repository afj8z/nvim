...
function M.leading_white_spaces(i)
	return luasnip.function_node(function(_, snip)
		return compute_leading_white_spaces(snip, i)
	end)
end
...
function M.visual(idx, default, line_prefix, indent_capture_idx)
	...
	return luasnip.dynamic_node(idx, function(_, snip)
		...
		if #select_raw > 0 then
			...
			local leading = ""
			if indent_capture_idx ~= nil then
				leading = compute_leading_white_spaces(snip, indent_capture_idx)
			end
...
function M.blocktransform(expand, insert, prepend, indent_capture_idx)
	...
	if not indent and not prepend then
